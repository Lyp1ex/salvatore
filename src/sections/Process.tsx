...
export default function Process({ site }: ProcessProps) {
  ...
  return (
    <section ref={sectionRef} id="process" className="scroll-mt-24 py-20">
      <motion.div
      ...
      >
        ...
        <div className="grid gap-5 lg:grid-cols-[1.25fr_.95fr]">
          <div className="relative overflow-hidden rounded-3xl border border-[rgba(212,176,93,.26)] bg-[linear-gradient(135deg,rgba(255,255,255,.06),rgba(10,11,14,.82))] p-5 shadow-[0_20px_70px_rgba(0,0,0,.48)] sm:p-6">
            <div className="absolute bottom-6 left-7 top-6 w-[2px] bg-white/10" />
            <motion.div
              style={{ scaleY: lineScale }}
              className="absolute bottom-6 left-7 top-6 w-[2px] origin-top bg-[linear-gradient(180deg,var(--lux-gold),var(--lux-mint))]"
            />
            ...
            <div className="space-y-4">
              {site.storyMoments.map((moment, index) => (
                <motion.article
                  ...
                  className="relative ml-4 rounded-2xl border border-white/10 bg-black/25 p-4"
                >
                  <span className="absolute -left-[26px] top-5 h-3 w-3 rounded-full border border-[var(--lux-gold-soft)] bg-[var(--lux-gold)] shadow-[0_0_16px_rgba(212,176,93,.5)]" />
