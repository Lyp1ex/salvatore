...
export default function Navbar({
  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-3 z-50 px-3 sm:px-6">
      <nav className="pointer-events-auto mx-auto flex max-w-6xl items-center gap-1 rounded-xl border border-[rgba(196,164,92,.22)] bg-[rgba(10,11,15,.78)] p-1.5 shadow-[0_14px_40px_rgba(0,0,0,.5)] backdrop-blur-xl">
        <div className="hidden items-center px-2 sm:flex">
          <span className="h-2 w-2 rounded-full bg-[var(--lux-gold)] shadow-[0_0_12px_rgba(196,164,92,.55)]" />
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <a
                ...
                href={item.href}
                data-cursor="active"
                className={`nav-pill relative shrink-0 rounded-lg px-2.5 py-1.5 text-[11px] font-medium transition duration-300 sm:px-3 sm:text-sm ${
                  isActive
                    ? "text-[var(--lux-cream)]"
                    : "text-zinc-300 hover:bg-[rgba(196,164,92,.14)] hover:text-[var(--lux-cream)]"
                }`}
              >
        ...
        <button
          ...
          className="shrink-0 rounded-lg border border-[rgba(212,176,93,.28)] px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.13em] text-zinc-300 transition hover:bg-[rgba(212,176,93,.14)] sm:px-3 sm:text-[11px]"
        >
